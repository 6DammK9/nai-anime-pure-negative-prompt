...
## Exploring full model training after hardware upgrade ##

- With the recent upgrade from [4x 3090 24G](../../ch04/4x3090_v3.md) to [4x V100 32G](../../ch04/4xV100_v4.md), finally I can gather the actual VRAM usage for full model training, and the corrosponding optimal setting. *I was temporally fooled by a fauly core (uncorrectable error in 21GB) and hit bs10, which was proven invalid when I used 3 cards instead.*

- VRAM usage: **26.1GB** (bs1), **31.9GB** (bs4).

- The CPU stalling is so severe, which **stalls per training step**, it could be relived by further increasing the batch size, but it will fall into not having full model weight training.

- Default: **100% UNET**, grad accu = 4, 4x GPU, **bs4**, **straight into training after power on** (to avoid cross CCD traffic), `adamW8bit` (which is fastest), 8x 64GB 2933Mhz 2Rx4 system memory

|CPU|Motherboard|GPU Power consumption (W)|Speed (img/s)|
|---|---|---|---|
|AMD EPYC 7F52|Tyan S8030|200-240|**3.953**|
