...
accelerate launch --num_cpu_threads_per_process=8 sdxl_train_v2.py --pretrained_model_name_or_path="/run/media/user/Intel P4510 3/astolfo_xl/just_astolfo/model_out_26011101/epoch-000008.safetensors" --in_json "/run/media/user/Intel P4510 3/just_astolfo/meta_lat.json" --train_data_dir="/run/media/user/Intel P4510 3/just_astolfo/kohyas_finetune" --output_dir="/run/media/user/Intel P4510 3/astolfo_xl/just_astolfo/model_out" --log_with=tensorboard --logging_dir="/run/media/user/Intel P4510 3/astolfo_xl/just_astolfo/tensorboard" --log_prefix=just_astolfo_26020101_ --seed=26020101 --save_model_as=safetensors --caption_extension=".txt" --enable_wildcard --use_8bit_adam --learning_rate="1e-5" --flow_model --flow_uniform_shift --contrastive_flow_matching --max_train_epochs=50 --xformers --pin_memory --gradient_checkpointing --gradient_accumulation_steps=4 --max_grad_norm=0 --max_data_loader_n_workers=8 --persistent_data_loader_workers --train_batch_size=1 --full_bf16 --mixed_precision=bf16 --save_precision=fp16 --enable_bucket --cache_latents --skip_cache_check --save_every_n_epochs=7
